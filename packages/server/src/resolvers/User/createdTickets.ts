...
export const createdTickets = (async (parent, input, { database, schema }) => {
  const { first, after } = await validateUsingSchema(inputSchema, input)

  return getPaginatedTickets(database, schema, first, after, eq(schema.tickets.authorId, parent.id))
}) satisfies UserResolvers['createdTickets']
